...
-- ----------------------------------------------------------------
-- view_creation.sql
-- View creation script for Oracle 12c and higher for the Delft-FEWS database schema (procedures, functions and views).
...
DROP FUNCTION &strusername..FromInterval;

-- Converts a time interval to fractional seconds
CREATE FUNCTION &strusername..FromInterval(i IN DSINTERVAL_UNCONSTRAINED)
  RETURN FLOAT AS
BEGIN
  RETURN ((EXTRACT(DAY FROM i) * 24 + EXTRACT(HOUR FROM i)) * 60 + EXTRACT(MINUTE FROM i)) * 60 + EXTRACT(SECOND FROM i);
END;
/
